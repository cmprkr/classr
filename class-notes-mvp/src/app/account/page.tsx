...
export default async function AccountPage() {
  ...
  const dbUser = session?.user?.id
    ? await db.user.findUnique({
        ...
        select: { id: true, name: true, email: true, image: true, username: true, planTier: true }, // <-- add planTier
      })
    : null;
  ...
  const isPremium = dbUser?.planTier === "PREMIUM"; // <-- derive
  ...
  return (
    <>
      ...
      <section className="relative flex-1">
        ...
        <AccountClient user={user} isSignedIn={!!session?.user} isPremium={isPremium} />
      </section>
    </>
  );
}
