...
export default function RecorderPanel() {
    ...
    fetch("/api/classes")
      ...
      .then((arr: Klass[]) => {
        ...
        if (classIdInPath && list.some((c) => c.id === classIdInPath)) {
          setTargetClassId(classIdInPath); // <- preselect current class
        } else if (list.length && !targetClassId) {
          setTargetClassId(list[0].id);
        }
      })
      .catch(() => {});
    // we only need to recalc when URL path changes
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [pathname]);
  ...
  function startVisualization(s: MediaStream) {
    stopVisualization(); // cleanup if any
  ...
  async function start() {
    try {
      ...
      setRecorder(rec);
      setState("recording");
      setRecActive(true); // <- ONLY true while recording
  ...
  function pauseRec() {
    ...
    if (recorder.state === "recording") {
      recorder.pause();
      setState("paused");
      setRecActive(false); // <- red dot OFF while paused
      // keep visualization running or stop? choose to keep it running:
      // (if you want to freeze the bars, you can call stopVisualization() here)
    }
  }
  ...
  function stopRec() {
    ...
    if (recorder.state !== "inactive") {
      recorder.stop();
      setRecorder(null);
      // visualization & status cleaned up in onstop
    }
  }
  ...
  useEffect(() => {
    ...
    const resize = () => {
      ...
      canvas.height = Math.max(60, Math.floor(120 * dpr)); // fixed visual height
