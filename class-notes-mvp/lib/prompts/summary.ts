...
export type SummaryPromptOptions = {
  basis: string;                   // first ~12k chars of source text
  includeSourceNotes?: boolean;    // include the "## Source Notes" section
  extraFooter?: string;            // anything else you want to append (rare)
};
...
/**
 * Build the user prompt content string.
 * Toggle “Source Notes” via includeSourceNotes, since one of your routes used it.
 */
export function buildSummaryUserContent(opts: SummaryPromptOptions): string {
  const lines = baseUserSections();

  if (opts.includeSourceNotes) {
    lines.push(
      "",
      "## Source Notes",
  ...
  lines.push(
    ...
    (opts.basis || "").slice(0, 12000)
  );

  if (opts.extraFooter?.trim()) {
    lines.push("", opts.extraFooter.trim());
  }
...
/**
 * Convenience: build the OpenAI chat messages array.
 */
export function buildSummaryMessages(opts: SummaryPromptOptions) {
  return [
    { role: "system", content: SUMMARY_SYSTEM },
    { role: "user", content: buildSummaryUserContent(opts) },
  ] as const;
}
